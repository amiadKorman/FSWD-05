...
const Todos = () => {
  ...
  useEffect(() => {
    if (currentUserId !== null) {
      fetch(apiUrl)
        ...
        .then((data) =>
          setTodos(data.filter((todo) => Number(todo.userId) === currentUserId))
        )
  ...
  const sortedTodos = filteredTodos.sort((a, b) => {
    ...
    if (sortCriteria === "done")
      return a.completed === b.completed ? 0 : a.completed ? -1 : 1;
    if (sortCriteria === "notDone")
      return a.completed === b.completed ? 0 : a.completed ? 1 : -1;
    return 0;
  });
      ...
      <ul>
        {searchResults.map((todo, index) => (
          <li key={todo.id}>
            ...
            />
            {index + 1}. ID: {todo.id}, User ID: {todo.userId}, {todo.title}{" "}
            {/* Serial number, ID, User ID, and title */}

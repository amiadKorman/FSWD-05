...
const Albums = () => {
  ...
  useEffect(() => {
    if (currentUserId !== null) {
      fetch(albumsApiUrl)
        ...
        .then((data) =>
          setAlbums(
            data.filter((album) => Number(album.userId) === currentUserId)
          )
        )
  ...
  const fetchPhotos = (albumId, page = 1) => {
    fetch(
      `${photosApiUrl}?albumId=${albumId}&_page=${page}&_limit=${photosPerPage}`
    )
      .then((response) => response.json())
      .then((data) => {
        if (page === 1) {
  ...
  return (
    <div>
      <h1>Albums</h1>
      <button onClick={() => addAlbum(prompt("New album title"))}>
        Add Album
      </button>
      <div>
        <input
      ...
      <ul>
        {searchResults.map((album, index) => (
          <li key={album.id}>
            ...
            <button
              onClick={() => navigate(`/user/${userId}/album/${album.id}`)}
            >
              Select
            </button>
            ...
            <button
              onClick={() =>
                updateAlbum(album.id, {
                  ...album,
                  title: prompt("New title", album.title),
                })
              }
            >
              Edit
                ...
                <button
                  onClick={() =>
                    updatePhoto(photo.id, {
                      ...photo,
                      title: prompt("New title", photo.title),
                      url: prompt("New URL", photo.url),
                      thumbnailUrl: prompt(
                        "New thumbnail URL",
                        photo.thumbnailUrl
                      ),
